...
export const gradientMaker = {
  ...
  _multiplierStopsAbsolute(stops, multiplier, start, offset) {
    return [...Array(stops)].map(
      (x, i, arr) => (arr[i] = offset + 100 * start * multiplier ** (i + 1))
    );
  },
  ...
  _multiplierStops(stops, multiplier) {
    ...
    return [...Array(stops)].map(
      (x, i, arr) =>
        (arr[i] =
          multiplier **
          ((i + 1 > halfStops ? even : 0) + i - Math.floor(halfStops)))
    );
  },
  _suffixise(arr) {
    return arr.map((x) => `-${x}`);
  ...
  _makeGradient(mainColour, stops) {
    if (stops < 2) {
      ...
      const satMultStops = this._multiplierStopsAbsolute(
        stops,
        satMult,
        this._satStart,
        satOffset
      );
      const lumMultStops = this._multiplierStopsAbsolute(
        stops,
        lumMult,
        this._lumStart,
        lumOffset
      ...
      const gradientStringStart = isPrimary
        ? `linear-gradient(to left`
        : `linear-gradient(to top, #000 ${gap}, ${mainColour.hex} ${gap}, ${mainColour.hex}) 0% 0% / 100% 70% no-repeat, linear-gradient(to left`;
      ...
      suffixes.forEach((suffix, i) => {
        const newColour = colourObject.assign(mainColour, {
          name: customName + suffix,
          lum: lumMultStops[i],
          sat: mainColour.name === "neutral" ? 0 : satMultStops[i],
          operation: "replace",
        });
