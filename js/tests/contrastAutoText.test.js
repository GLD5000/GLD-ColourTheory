...
function testLoop() {
  ...
  function testLoopB() {
    for (let lum = 0.022122; lum < 1; lum += 0.01) {
      ...
      const returnColour = colourObject.fromHex({hex: contrast._autoTextColour(backgroundColour, 7)[0]});
      ...
      return text colour: ${returnColour.hex}`, () => {
        expect(Math.round(contrast.getContrastRatio([returnColour.relativeLuminance, backgroundColour.relativeLuminance]))).toBeGreaterThan(
            minContrast
        );
      });
    }
  ...
  function testLoopC() {
    for (let lum = 0.022122; lum < 1; lum += 0.01) {
      ...
      test(`Test _convertLuminanceToHex return colour luminance ${lum}`, () => {
        expect(returnColour.relativeLuminance.toFixed(1)).toBe(
            lum.toFixed(1)
        );
      });
    }
